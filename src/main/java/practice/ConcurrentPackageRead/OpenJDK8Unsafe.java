/*
 * Copyright (c) 2000, 2013, Oracle and/or its affiliates. All rights reserved. DO NOT ALTER OR REMOVE COPYRIGHT NOTICES
...
public class OpenJDK8Unsafe {
     ...
     * <p>
     * <p>
     * The returned <code>Unsafe</code> object should be carefully guarded by the caller, since it can be used to read
     * and write data at arbitrary memory addresses. It must never be passed to untrusted code.
     * <p>
     * <p>
     * Most methods in this class are very low-level, and correspond to a small number of hardware instructions (on
     * typical machines). Compilers are encouraged to optimize these methods accordingly.
     * <p>
     * <p>
     ...
     *
     * @throws SecurityException if a security manager exists and its <code>checkPropertiesAccess</code> method
     *                           doesn't allow access to the system properties.
     */
    @CallerSensitive
     ...
     * <ul>
     * <li>The offset was obtained from {@link #objectFieldOffset} on the {@link java.lang.reflect.Field} of some Java
     * field and the object referred to by <code>o</code> is of a class compatible with that field's class.
     * <p>
     ...
     * <code>B+N*S</code>, where <code>N</code> is a valid index into the array, and <code>B</code> and <code>S</code>
     * are the values obtained by {@link #arrayBaseOffset} and {@link #arrayIndexScale} (respectively) from the array's
     * class. The value referred to is the <code>N</code><em>th</em> element of the array.
     * <p>
     * </ul>
     * <p>
     ...
     * <em>double-register</em> addressing mode for Java variables. When the object reference is null, this method uses
     * its offset as an absolute address. This is similar in operation to methods such as {@link #getInt(long)}, which
     * provide (in effect) a <em>single-register</em> addressing mode for non-Java variables. However, because Java
     * variables may have a different layout in memory from non-Java variables, programmers should not assume that these
     * two addressing modes are ever equivalent. Also, programmers should remember that offsets from the double-register
     * addressing mode cannot be portably confused with longs used in the single-register addressing mode.
     *
     * @param o      Java heap object in which the variable resides, if any, else null
     * @param offset indication of where the variable resides in a Java heap object, if any, else a memory address
     *               locating the variable statically
     ...
     * Java variable (field or array element). The given value is stored into that variable.
     * <p>
     ...
     * <p>
     * Unless the reference <code>x</code> being stored is either null or matches the field type, the results are
     * undefined. If the reference <code>o</code> is non-null, car marks or other store barriers for that object (if the
     * VM requires them) are updated.
     *
    ...
    /**
     * This method, like all others with 32-bit offsets, was native in a previous release but is now a wrapper which
     * simply casts the offset to a long value. It provides backward compatibility with bytecodes compiled against 1.4.
     *
    ...
    /**
     * Fetches a native pointer from a given memory address. If the address is zero, or does not point into a block
     * obtained from {@link #allocateMemory}, the results are undefined.
     * <p>
     * <p>
     * If the native pointer is less than 64 bits wide, it is extended as an unsigned number to a Java long. The pointer
     * may be indexed by any given byte offset, simply by adding that offset (as a simple integer) to the long
     * representing the pointer. The number of bytes actually read from the target address maybe determined by
     * consulting {@link #addressSize}.
     *
     * @see #allocateMemory
     */
    ...
    /**
     * Stores a native pointer into a given memory address. If the address is zero, or does not point into a block
     * obtained from {@link #allocateMemory}, the results are undefined.
     * <p>
     * <p>
     ...
     * they will generally be garbage. The resulting native pointer will never be zero, and will be aligned for all
     * value types. Dispose of this memory by calling {@link #freeMemory}, or resize it with {@link #reallocateMemory}.
     *
    ...
    /**
     * Resizes a new block of native memory, to the given size in bytes. The contents of the new block past the size of
     * the old block are uninitialized; they will generally be garbage. The resulting native pointer will be zero if and
     * only if the requested size is zero. The resulting native pointer will be aligned for all value types. Dispose of
     * this memory by calling {@link #freeMemory}, or resize it with {@link #reallocateMemory}. The address passed to
     ...
     * <p>
     * <p>
     * This method determines a block's base address by means of two parameters, and so it provides (in effect) a
     * <em>double-register</em> addressing mode, as discussed in {@link #getInt(Object, long)}. When the object reference
     * is null, the offset supplies an absolute base address.
     * <p>
     * <p>
     * The stores are in coherent (atomic) units of a size determined by the address and length parameters. If the
     * effective address and length are all even modulo 8, the stores take place in 'long' units. If the effective
     * address and length are (resp.) even modulo 4 or 2, the stores take place in units of 'int' or 'short'.
     *
     * @since 1.7
     */
     ...
     * <p>
     * <p>
     * This method determines each block's base address by means of two parameters, and so it provides (in effect) a
     * <em>double-register</em> addressing mode, as discussed in {@link #getInt(Object, long)}. When the object reference
     * is null, the offset supplies an absolute base address.
     * <p>
     * <p>
     * The transfers are in coherent (atomic) units of a size determined by the address and length parameters. If the
     * effective addresses and length are all even modulo 8, the transfer takes place in 'long' units. If the effective
     * addresses and length are (resp.) even modulo 4 or 2, the transfer takes place in units of 'int' or 'short'.
     *
     * @since 1.7
     */
    ...
    /**
     * Sets all bytes in a given block of memory to a copy of another block. This provides a <em>single-register</em>
     * addressing mode, as discussed in {@link #getInt(Object, long)}.
     * <p>
    ...
    /**
     * Returns the base address for accessing some static field in the given class. This method is implemented as
     * follows: <blockquote>
     * <p>
     * <pre>
     ...
     * {@link Field}. This method works only for JVMs which store all statics for a given class in one
     * place.
     */
    @Deprecated
    public Object staticFieldBase(Class<?> c) {
     ...
     * Therefore, code which will be ported to such JVMs on 64-bit platforms must preserve all bits of static field
     * offsets.
     *
     * @see #getInt(Object, long)
     */
     ...
     * within a non-array object, However, for consistency with other methods in this class, this method reports its
     * result as a long value.
     *
     * @see #getInt(Object, long)
     */
     ...
     * guaranteed to be a real Object, and it should not be used in any way except as argument to the get and put
     * routines in this class.
     */
    public native Object staticFieldBase(Field f);

    /**
     * Detect if the given class may need to be initialized. This is often needed in conjunction with obtaining the
     * static field base of a class.
     *
    ...
    /**
     * Ensure the given class has been initialized. This is often needed in conjunction with obtaining the static field
     * base of a class.
     */
     ...
     * {@link #arrayIndexScale} returns a non-zero value for the same class, you may use that scale factor, together
     * with this base offset, to form new offsets to access elements of arrays of the given class.
     *
     * @see #getInt(Object, long)
    ...
    /**
     * Report the scale factor for addressing elements in the storage allocation of a given array class. However, arrays
     * of "narrow" types will generally not work properly with accessors like {@link #getByte(Object, int)}, so the
    ...
    /**
     * Report the size in bytes of a native pointer, as stored via {@link #putAddress}. This value will be either 4 or
     * 8. Note that the sizes of other primitive types (as stored in native memory blocks) is determined fully by their
     * information content.
     */
    ...
    /**
     * Tell the VM to define a class, without security checks. By default, the class loader and protection domain come
     * from the caller's class.
     */
    ...
    /**
     * Tries to lock the object. Returns true or false to indicate whether the lock succeeded. If it did, the object
     * must be unlocked via {@link #monitorExit}.
     */
    ...
    /**
     * Version of {@link #putObjectVolatile(Object, long, Object)} that does not guarantee immediate visibility of the
     * store to other threads. This method is generally only useful if the underlying field is a Java volatile (or if an
     ...
     * <tt>park</tt> not to block. Note: this operation is "unsafe" solely because the caller must somehow ensure that
     * the thread has not been destroyed. Nothing special is usually required to ensure this when called from Java (in
     * which there will ordinarily be a live reference to the thread) but this is not nearly-automatically so when
    ...
    /**
     * Block current thread, returning when a balancing <tt>unpark</tt> occurs, or a balancing <tt>unpark</tt> has
     * already occurred, or the thread is interrupted, or, if not absolute and time is not zero, the given time
     * nanoseconds have elapsed, or if absolute, the given deadline in milliseconds since Epoch has passed, or
     * spuriously (i.e., returning for no "reason"). Note: This operation is in the Unsafe class only because
     * <tt>unpark</tt> is, so it would be strange to place it elsewhere.
     */
    public native void park(boolean isAbsolute, long time);

    /**
     * Gets the load average in the system run queue assigned to the available processors averaged over various periods
     * of time. This method retrieves the given <tt>nelem</tt> samples and assigns to the elements of the given
     * <tt>loadavg</tt> array. The system imposes a maximum of 3 samples, representing averages over the last 1, 5, and
     ...
     * <code>o</code> at the given <code>offset</code>.
     *
    ...
    /**
     * Atomically exchanges the given reference value with the current reference value of a field or array element
     * within the given object <code>o</code> at the given <code>offset</code>.
     *
     * @param o        object/array to update the field/element in
     * @param offset   field/element offset
     * @param newValue new value
